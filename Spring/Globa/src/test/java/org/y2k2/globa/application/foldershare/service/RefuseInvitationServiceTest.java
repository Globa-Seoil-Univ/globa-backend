...
public class RefuseInvitationServiceTest {
    ...
    @Test
    @DisplayName("초대 거절 - 실패 (초대 상태가 PENDING이 아님)")
    public void refuseInvitationNotPending() {
        Long folderId = 1L;
        Long shareId = 1L;
        Long userId = 1L;
        ...
        Assertions
                .assertThatThrownBy(() -> refuseInvitationService.refuse(folderId, shareId, userId))
                .isInstanceOf(CustomException.class)
                .hasFieldOrPropertyWithValue("errorCode", ErrorCode.SHARE_INVITATION_NOT_PENDING);
